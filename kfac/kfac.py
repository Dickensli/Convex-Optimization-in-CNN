...
class KFAC(Optimizer):
    def __init__(self, model, loss_func, approx_type='diagonal', lam=0.2, eta=1e-5, T_2=20,
                 update_gamma_every=20, update_lam_every=5, update_approx_every=20, sample_ratio=1/4):
        self.kfac_params = {}
        ...
        if self.layers[0].weight.is_cuda:
            self.device = self.layers[0].weight.get_device()
    ...
    def step(self, batch_loss):
        ...
        self.prev_loss = batch_loss.data
        self.n_iter += 1
    ...
    def jvp(self, v):
        if len(v.size()) > 1:
            jvp = torch.zeros(*self.log_output_dist.size(), v.size()[1]).to(self.device)
        else:
            jvp = torch.zeros(self.log_output_dist.size()).to(self.device)

        for i in range(jvp.size()[0]):
            for j in range(jvp.size()[1]):
                grads_unrolled = []
                self.model.zero_grad()
                # Check if we can get rid of the computational graph. This is the last backprop in the KFAC optimization step
                retain_graph = i < jvp.size()[0] - 1 or j < jvp.size()[1] - 1
                # Get the row of the Jacobian for the ith batch corresponding to the jth output
                grads = torch.autograd.grad(self.final_preacts[i][j], self.model.parameters(), retain_graph=retain_graph)
